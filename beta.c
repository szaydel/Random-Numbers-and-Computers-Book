...
//  file:  beta.c
//
//  Code to sample from a beta distribution given
...
double beta_gc(double a, double b) {
    double x = gamma_gc(a,1);
    return x / (x + gamma_gc(b,1));
}


double beta_ms(double a, double b) {
    double x = gamma_ms(a,1);
    return x / (x + gamma_ms(b,1));
}
