...
#define SFIELD	23
#define STABSZ	3000000 /* was 6000 */
#define SBOLSZ	20
...
int		first_write=1;		/* first write to output file */
unsigned int	file_loc;		/* gets location counter on first write, 
...
int		lflag=-1;		/* disable listing flag */
int		sflag;			/* do a symbol table print at end of listing */
unsigned int	loccnt;			/* location counter	*/
int		nxt_free;		/* next free location in symtab */
int		oflag=-1;		/* object output flag */
int		opflg;			/* operation code flags */
int		opval;			/* operation code value */
int		pass;			/* pass counter		*/
char		prlnbuf[LAST_CH_POS+1]; /* print line buffer	*/
unsigned int	slnum;			/* source line number counter */
	...
	while (--argc > 0 && (*++argv)[0] == '-') {
		for (i = 1; (c = tolower((*argv)[i])) != '\0'; i++) {
			...
			if (c == 'v')		/* print assembler version */
				fprintf(stdout, "as6502 - July 7, 1998\n");
		}
	}
	ac = argc;
	av = argv;
   ...
   int  argc;
{
	...
	if ((pass == LAST_PASS) && (oflag != 0) && ac == argc) {
		if ((binptr = fopen("6502.bin", "wb")) == NULL) {
			fprintf(stdout, "Can't create 6502.bin.\n");
			exit(1);
		}
	}
}
    ...
    int outflg;
{
	...
	if ((pass == LAST_PASS) && (oflag != 0) && (outflg != 0)) {
		...
		if (file_loc > loccnt) {
			...
			fprintf(stdout, "%s\n", prlnbuf);
			fprintf(stdout, "Can't relocate backwards.\n");
			errcnt++;
			return;
		}
		while (file_loc < loccnt) {
			binputc(0, binptr);
			++file_loc;
		}
	}
